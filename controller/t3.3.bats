...
@test "3.3.1 - We can create a user on the system" {
   sshpass -p system ssh login.vc-a obol -H ldap://controller -w system group add users
   sshpass -p system ssh login.vc-a obol -H ldap://controller -w system user add --password 123 --sn "Smith" john
   sshpass -p system ssh login.vc-a obol -H ldap://controller -w system user add --password 123 --sn "Jane" --groups power-users jane
   sshpass -p system ssh login.vc-a obol -H ldap://controller -w system group show users
   sshpass -p system ssh login.vc-a obol -H ldap://controller -w system user show john
   sshpass -p system ssh login.vc-a obol -H ldap://controller -w system user delete john
   sshpass -p system ssh login.vc-a obol -H ldap://controller -w system user list | grep -v john
}
